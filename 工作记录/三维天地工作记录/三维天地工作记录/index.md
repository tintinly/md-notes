...
![image-20220725172021025](assets\image-20220725172021025.png)

![image-20220725172048567](assets\image-20220725172048567.png)
...
项目的文件基本上都放在 module 文件夹下，framework 文件包放的是一些工具类，bp 包下放的是主业务流程相关文件，index 放的是项目项目首页配置和一些其他功能的配置，只要体现在 js 上，item 包下放的是除主业务流程和静态数据相关的其他功能的文件，mdm 放的是静态数据相关的文件，剩下的放的比较乱
 ...
 ![image-20220706090553363](assets\image-20220706090553363.png)

 

 这个是五个静态数据

![image-20220706090612919](assets\image-20220706090612919.png)

 这个是五个主业务流程
 你可以结合昨天我发你的培训资料里的东西先看看，配合页面更直观

 ![image-20220706090631023](assets\image-20220706090631023.png)
...
给你个需求，需要编制审核查询三个页面，这三个页面的字段都一样只有按钮功能不一样，审核页只有审核按钮，查询页没有按钮
 ...
 ![image-20260615230431496](assets/image-20260615230431496.png)



 一个功能模块的结构，resource 这一层是接收页面的请求并将请求传给 service

 ![image-20260615225951439](assets/image-20260615225951439.png)
...
![image-20220706090844483](assets\image-20220706090844483.png)

![image-20220706090901951](assets\image-20220706090901951.png)

![image-20220707094049673](assets\image-20220707094049673.png)

icon 就不用说了，i18n 是字段国际化，config 里面是配置下拉框或者弹出选择框的

![image-20220707094101147](assets\image-20220707094101147.png)
...
![image-20220707094238447](assets\image-20220707094238447.png)

这种是下拉框的写法

![image-20220707094301797](assets\image-20220707094301797.png)

onBeforeChoose 可以给弹出框传递参数，onAfterChoose 是接收弹出框的返回值并复制给需要这个返回值的数据，onClean 是监测单元格是否情况的，这种带弹出框的单元格会有一个 x 的图标，点了之后会触发这个方法

![img](assets\Q2M9QRA5L}}OBXMH}A{6{DF.png)

这个地方有两个和 emc 平级的 gikam 文件夹和 module 文件夹，不用管就行，用不到

![image-20220707094331519](assets\image-20220707094331519.png)

有新增的 sql 的话就这个文件夹里面就行，里面有格式，你照着格式写就 ok

![image-20220707134212088](assets\image-20220707134212088.png)

还有就是 mapper 的格式，where 的 include 这个是必须加的因为这个是工作流附带的条件语句，when 的 include 的作用是前端配置的排序规则可以直接自动拼接 sql

![img](assets\QOGL$[FUPIQ8NC`CFF8RQJU.png)
这个方法是获取提交工作流的
![image-20220711101454011](assets\image-20220711101454011.png)
核心是这么写的

![img](assets\5DVYMCA7[8Q{IWAQ2WBKC@8.png)
通过这个应该是可以直接获取到 common 的，如果这个不行的话就直接从 wrapper 里面截取审核意见就行

一般没人这么存，都是链接的表查的，但是链接表的话又会把 sql 搞的很慢

![image-20220711101515455](assets\image-20220711101515455.png)
这个是获取通过的工作流的
...
![image-20220711150421656](assets\image-20220711150421656.png)
...
![image-20220711165336523](assets\image-20220711165336523.png)
...
![image-20220725143753619](assets\image-20220725143753619.png)

* select 下拉框 由 select-config.js 指定查询的接口

![image-20220725143844691](assets\image-20220725143844691.png)
...
![image-20220811165404325](assets\image-20220811165404325.png)
...
![image-20220826092013689](assets\image-20220826092013689.png)
...
![image-20221014151020810](assets\image-20221014151020810.png)

![image-20221014151101018](assets\image-20221014151101018.png)
...
![image-20221115210616373](assets\image-20221115210616373.png)
...
![image-20260615230436106](assets/image-20260615230436106.png)
...
![image-20220920103141052](assets\image-20220920103141052.png)

* 自定义修约规则 T_CORE_ROUNDING_RULE

![image-20220926180347588](assets\image-20220926180347588.png)
...
![image-20221212132522652](assets\image-20221212132522652.png)

![image-20221212132538699](assets\image-20221212132538699.png)
...
![image-20260501173537064](assets/image-20260501173537064.png)
...
![image-20220725143340738](assets\image-20220725143340738.png)

![image-20220725143446224](assets\image-20220725143446224.png)
...
![image-20221124152109855](assets\image-20221124152109855.png)
...
![image-20220715105353007](assets\image-20220715105353007.png)
...
![image-20220803162103039](assets\image-20220803162103039.png)

* 连接公司数据库需要 vpn

* 指定下一审核人

![image-20220916184148338](assets\image-20220916184148338.png)
...
![image-20221013083544868](assets\image-20221013083544868.png)

* 分析结果维护的函数类 ResultFormulaRepository

![image-20221013103957087](assets\image-20221013103957087.png)
...
![image-20221219143801810](assets\image-20221219143801810.png)
