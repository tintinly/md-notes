...
![img](assets/v2-386637e552bc20dd6298a4cb71896a9d_r.jpg)

* 软路由作为旁路由

![img](assets/v2-977baf841991fe207b45e575348537b5_720w.webp)
...
![image-20211123172504055](assets\image-20211123172504055.png)
...
![image-20250826213705732](assets\image-20250826213705732.png)
...
同源策略（Same-Origin Policy, SOP）是浏览器的一种安全机制

定义：限制从一个源加载的文档（脚本）与另一个源的资源进行交互的能力。
...
![image-20250827011117942](assets\image-20250827011117942.png)
...
以下均以搭建一个前端UI来访问 DockersRegistry 为例
...
1. CORS机制
   ...
   3. 临时解决（适合开发）。如 Chrome 的“Allow CORS”插件临时通过CORS 机制来显式允许跨域访问。

2. 搭建后端代理（可靠但又加了无谓的一层）。例，实现一个简单的 API 转发服务后端，由后端调用 Registry，再由前端访问后端。
...
WebSocket 核心是 “一次连接，双向持续对话”，解决了传统 HTTP 协议 “只能客户端问、服务器答” 的单向通信问题，就像从 “发短信” 升级到 “打电话”，适合需要实时数据交互的场景，比如聊天、实时监控、在线协作工具。

```nginx
# 实现 WebSocket 代理的 Nginx 配置
