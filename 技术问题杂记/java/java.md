...
> 在java中，散列函数实际上是返回一个int整数。这个哈希码的作用就是用来确定该对象在哈希表中的索引位置。假如散列表是一个长度有限的数组，可能会出现哈希冲突（两个不同对象计算出来的哈希表索引位置相同）。链地址法就是一种解决哈希冲突的方法，位置重复的元素与旧元素通过equal()确定唯一性后，会接到旧元素的（7前8后）面。
