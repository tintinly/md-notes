...
* **数据**：描述客观事务的符号，是能在计算机中识别并输入给计算机操作的符号集合。包括数值类型（整型数据和实型数据）和非数值类型（图片、视频、声音等，可以通过编码的手段编程字符数据来处理）。
* **数据元素**：组成数据的、有意义的**基本单位**，被计算机作为整体处理。也成为**记录**。比如人。
...
![image-20251002122312237](assets/image-20251002122312237.png)

> 当你告诉计算机要建立一个有9个整型数据的数组时，计算机就在内存中找了片空地,按照—个整型所占位置的大小乘以9开辟—段连续 的空间，第一个数据放在第一个位置，于是这样按顺序依次摆放。

2. 链式存储结构

当数据结构处于变化之中，比如移除旧元素、增加新元素，顺序存储就不够科学了。
...
![image-20251002122258374](assets/image-20251002122258374.png)

## 1.3 数据类型

数据类型的定义仅取决于它的—组逻辑特性,而与其在计算机内部如何表示和实现无关。
...
> JAVA的一大特性——封装，定义一个类的属性和方法（行为），只对外提供必要的行为接口。就是这样的程序设计思想
>
> 提升代码的安全性、可维护性

![image-20251002125314984](assets/image-20251002125314984.png)
...
![image-20260615230017630](assets/image-20260615230017630.png)

![image-20260615230014299](assets/image-20260615230014299.png)
