...
运行时的多态性（后绑定）：依赖于继承机制和方法重写。我们可以使用父类类型的引用来引用和操作不同的子类对象，而具体执行哪个子类的方法则取决于对象的实际类型。Java虚拟机(JVM)会根据对象的实际类型来调用相应的方法。
...
> 抽象类可以有具体的实现方法的，也可以有抽象方法（有abstract修饰的方法）
...
javac编译后，编译器会自动帮助我们生成一个与枚举相关的类。自动完善静态的values()方法、静态的valueOf()方法、私有构造器和各个枚举实例。
...
被final修饰的变量成为常量，必须显示指定初始值，且不可更改。在编译阶段会存入常量池中。

被final修饰的成员变量为成员常量，通过和static一起来创建静态（类）常量，方法块中的常量称为局部常量。
...
final方法不可被重写，final方法的主要目的是防止该方法的内容被修改，JVM会尝试将其内联,以提高运行效率。
