...
问题：git 不区分文件名大小写。例如本地项目的文件是大写文件名，推送以后在远程仓库中查看时小写文件名，另起一个项目拉取代码得到的也是小写文件名，这使得项目的编写、构建、维护容易由于文件名不一致而产生奇奇怪怪得问题。

解决方法一：

```bash
git mv test.txt TEST.txt # 此时会报错
# 因此先改名为 temp.txt 再改成需要得文件名
git mv test.txt temp.txt
git mv temp.txt TEST.txt
```

解决办法二：

分两步，将待改名的文件先备份到其他文件夹内 → 提交代码 →  然后再还原到原本的目录下更改大小写 → 提交代码。
...
这种情况修改大小写，会被识别为 `untracked` 类 型的变更，如果直接推送到远程的话，那么远程仓库会同时存在大小写两个版本的文件（本地一切正常）