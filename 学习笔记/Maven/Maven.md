...
* 中央仓库： Maven官方维护的远程仓库，里面包含了这个世界上绝大多数流行的开源 Java 类库，以及源码、作者信息、许可证信息等等。
...
每个 phase 会执行自己默认的一个或多个 goal。goal 是最小任务单元。

例 `mvn compile`，将执行 `compile` 这个 phase，这个 phase 会调用 `compiler` 插件执行关联的 `compiler:compile` 这个 goal。
...
插件是 Maven 的核心功能，Maven 实际上是一个依赖插件执行的框架，每个任务实际上是由插件完成。Maven 插件通常被用来：
...
**Apache Maven 提供的 org.apache.maven.plugins 插件**：
...
* [1] [maven配置settings.xml 配置详解 - hanease - 博客园](https://www.cnblogs.com/hanease/p/19216401)
