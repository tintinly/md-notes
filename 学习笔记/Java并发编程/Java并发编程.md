...
[Java 内存模型（Java Memory Model, JMM）是 Java 虚拟机（JVM）的一部分](../JVM/JVM.md#Java 内存模型)，其核心使命是解决多线程环境下由 CPU 缓存和指令重排序引发的三大难题：**原子性、可见性和有序性**。
...
[设计模式之单例模式的实现](../设计模式/设计模式.md#创建型 - 单例模式 Singleton )
...
几种锁会随着竞争情况逐渐升级，[锁的升级](../JVM/JVM.md#锁优化) 很容易发生
...
每个 Java 对象都有 [对象头](../JVM/JVM.md#对象的内存布局)。如果是非数组类型，则用 2 个字宽来存储对象头，如果是数组，则会用 3 个字宽来存储对象头。在 32 位处理器中，一个字宽是 32 位；在 64 位虚拟机中，一个字宽是 64 位。
...
[同步代码块依赖于同步指令，同步方法依赖于运行时常量池中的 `ACC_SYNCHRONIZED` 标志](../JVM/JVM.md#同步指令)
...
java.util 包下提供了一些容器类（[集合框架](../Java集合/Java集合.md)），其中 Vector 和 Hashtable 是线程安全的，但实现方式比较粗暴，通过在方法上加 [sychronized](#synchronized 关键字) 关键字实现。
...
[参考](../Java集合/Java集合.md#CopyOnWriteList)
